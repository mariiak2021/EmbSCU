...
class MCCFormers_S(nn.Module):
  ...
  def __init__(self, feature_dim, h, w, d_model = 512, n_head = 4, n_layers = 2, dim_feedforward = 2048):
    ...
    self.linear_layer3 = nn.Linear(1024, 20)
  ...
  def forward(self, img1, img2, img_feat1, img_feat2, obj_fea_bef, obj_fea_aft,class_bef,class_aft):
    ...
    batch = img1.size(0)
    print (img1.shape)
    feature_dim = img1.size(1)
    h, w = img1.size(2), img1.size(3)

    d_model = self.d_model
    #print (batch)
    img1 = self.input_proj(img1)
    print (img1.shape)
    img2 = self.input_proj(img2)
    img1 = img1.view(batch, d_model, -1) #(batch, d_model, h*w)
    img2 = img2.view(batch, d_model, -1) #(batch, d_model, h*w)
    #h, w = panorama_shape[0], panorama_shape[1]
    img1 = img1.permute(2, 0, 1) #self.linear_layer3(img1).permute(2, 0, 1)
    img2 = img2.permute(2, 0, 1) #self.linear_layer3(img2).permute(2, 0, 1)
    ...
    print ("img_feat1 aft", img_feat1.shape, img1.shape) #torch.Size([20, 1, 512]) torch.Size([1, 512, 1024])
    img_feat_bef = torch.cat([img_feat1, img1], dim=0)
    img_feat_aft = torch.cat([img_feat2, img2], dim=0)
    img_feat_cat = torch.cat([img_feat_bef, img_feat_aft], dim=0)  # (batch, d_model, 2*h*w)
    #img_feat_cat = img_feat_cat.permute(2, 0, 1)  # (2*h*w, batch, d_model)
    print (img_feat_cat.shape)
    # Create index tensor for idx_embedding
    idx1 = torch.zeros(batch, 1044).long().to(device)
    idx2 = torch.ones(batch, 1044).long().to(device)
